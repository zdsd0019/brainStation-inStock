	...
	return (
		<div className="inventory__card">
			<Link to={`/inventory/${props.id}`} className="warehouseList__link">
				<div className={descriptionContainerClassName + ' inventory__description-container--name'}>
					<div className="inventory__header3-container">
						<h3 className={headerClassName}>ITEM</h3>
						<DropdownButton
							itemId={props.id}
							removeButtonClassName={'inventory__remove inventory__remove--mobile'}
						/>
					</div>
					<div>
						<p className="inventory__product-name">{name}</p>
						<p className="inventory__paragraph inventory__paragraph--description">{description}</p>
					</div>
				</div>
			</Link>
			...
			<div className="inventory__tablet-button-container">
				<DropdownButton
					...
					itemId={props.id}
					removeButtonClassName={removeButtonClassName + ' inventory__remove--tablet'}
				/>
			</div>
		</div>
	);
